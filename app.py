...
# Aqui já definimos que a barra começa expandida no PC (no celular o Streamlit encolhe ela sozinho)
...
# --- INJEÇÃO DE CÓDIGO NO <HEAD> PAI (ADSENSE, ANALYTICS E OVERRIDE DO BOTÃO) ---
injections = """
<script>
  // 1. Google Analytics
  const gaScript = window.parent.document.createElement('script');
  gaScript.src = "https://www.googletagmanager.com/gtag/js?id=G-XXXXXXXXXX";
  gaScript.async = true;
  window.parent.document.head.appendChild(gaScript);

  const gaInline = window.parent.document.createElement('script');
  gaInline.innerHTML = `
  ...
  // 2. Google AdSense (Garantido)
  const adsenseScript = window.parent.document.createElement('script');
  adsenseScript.src = "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-2570721553689369";
  adsenseScript.async = true;
  adsenseScript.crossOrigin = "anonymous";
  window.parent.document.head.appendChild(adsenseScript);

  // 3. OPÇÃO NUCLEAR: Forçar estilo via JavaScript DOM (Impede o Streamlit de bloquear)
  const forceButtonStyle = () => {
      // Procura o botão de todas as formas possíveis que o Streamlit o nomeia
      const btns = window.parent.document.querySelectorAll('[data-testid="collapsedControl"], button[kind="header"]');
      
      btns.forEach(btn => {
          // Força o fundo azul e formato de botão de app
          btn.style.setProperty('background-color', '#3B82F6', 'important');
          btn.style.setProperty('border-radius', '8px', 'important');
          btn.style.setProperty('margin', '15px', 'important');
          btn.style.setProperty('padding', '8px', 'important');
          btn.style.setProperty('box-shadow', '0 4px 6px rgba(0,0,0,0.3)', 'important');
          btn.style.setProperty('opacity', '1', 'important');
          
          // Pinta as setas de branco puro
  ...
  // O MutationObserver vigia a tela em tempo real. Se o Streamlit tentar "piscar" e voltar o botão pra cinza, o JS pinta de azul de novo na mesma hora.
...
"""
components.html(injections, height=0, width=0)
...
# --- INJEÇÃO DE CSS (BLINDADA PARA O BOTÃO) ---
st.markdown("""
    <style>
        /* 1. Oculta botões de sistema do Streamlit */
        header .stAppDeployButton { display: none !important; }
        [data-testid="stHeaderActionElements"] { display: none !important; }
        header[data-testid="stHeader"] { background-color: transparent !important; }
        footer { visibility: hidden !important; }
        
        /* 2. ESTILIZANDO O BOTÃO NATIVO DO MENU (ESTILO 4DEVS) - MIRA AMPLA */
        /* Pegamos todas as formas que o Streamlit chama esse botão para garantir */
        button[kind="header"], 
        ...
        [data-testid="stSidebarCollapsedControl"] {
            ...
            opacity: 1 !important; /* Mata o efeito translúcido/cinza */
            visibility: visible !important;
            box-shadow: 0 4px 6px rgba(0,0,0,0.2) !important; /* Sombra de app */
        ...
        /* Forçando a cor BRANCA no ícone (>> ou hambúrguer) */
        ...
        /* 3. Ajuste de margens do conteúdo principal */
        .block-container { padding-top: 2rem !important; max-width: 1050px; }
        [data-testid="stSidebar"] > div:first-child { padding-top: 0rem !important; }
        
        /* 4. Design da Barra Lateral */
        [data-testid="stSidebar"] { background-color: #0F172A !important; border-right: 1px solid #1E293B !important; }
        [data-testid="stSidebar"] * { color: #F8FAFC !important; }
        
        /* 5. Estilo dos botões do menu */
        [data-testid="stSidebar"] div.stButton > button {
            text-align: left; justify-content: flex-start; background-color: transparent; border: none; padding: 2px 10px; font-weight: 500; font-size: 15px; width: 100%; border-radius: 8px; transition: 0.3s;
        }
        [data-testid="stSidebar"] div.stButton > button:hover { background-color: #1E293B; color: #3B82F6 !important; }
        
        /* 6. Inputs e Caixas de Texto da Ferramenta */
        ...
        .recibo-box {
            ...
            margin-top: 20px; line-height: 1.6; overflow-x: auto;
        }
        .info-box-premium {
            background-color: #EFF6FF; border-left: 5px solid #3B82F6; padding: 20px; border-radius: 5px; color: #1E293B;
            margin-bottom: 25px; line-height: 1.6; font-size: clamp(14px, 2vw, 16px);
        }
...
# ==========================================
# TOPO DO SITE (BUSCA INTELIGENTE E BOTÕES)
# ==========================================
